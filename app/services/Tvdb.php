...
class Tvdb {
    ...
    public function __construct($apikey=null) {
        if(isset($apikey)) $this->api_key = $apikey;
        else {
    ...
    public function findSeries($seriename) {
        ...
        if( $feed ) {
            ...
            foreach($node as $series) {
                ...
                if(isset($series->IMDB_ID)) $s->setIMDBId($series->IMDB_ID);
                if(isset($series->FirstAired)) $s->setFirstAired($series->FirstAired);
                $result[] = $s;
            }
            return $result;
        } else {
...
//          $serie = new Default_Model_Series();
//          $serie->setTVDBId($serieid);
//            $serie->setName( (string) $xml->Series->SeriesName );
//            $serie->setDescription( (string) $xml->Series->Overview );
//            $serie->setIMDBId( (string) $xml->Series->IMDB_ID );
//            $serie->setFirstAired( (string) $xml->Series->FirstAired );
    ...
    private static function DownloadUrl($url, $timeout=3) {
        ...
        if( substr($url, -3) == 'zip') {
            $temp_dir  = APPLICATION_PATH . '/../tmp/' . self::getUniqueCode() . '/';
